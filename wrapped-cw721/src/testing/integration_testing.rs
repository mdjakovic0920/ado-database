...
use andromeda_std::{
    ado_base::ownership::ContractOwnerResponse, amp::AndrAddr, common::encode_binary,
...
fn wrapped_cw721_test() {
    ...
    let origin_nft_addr: Addr = router
    ...
    let contract_instantiate_msg: InstantiateMsg = InstantiateMsg {
        ...
        authorized_token_addresses: Some(vec![AndrAddr::from_string(origin_nft_addr.to_string())]),
    };
    let contract_addr: Addr = router
        .instantiate_contract(
            contract_id,
            addr1.clone(),
    ...
    let wrapped_nft_addr: Addr = router
        .instantiate_contract(
            wrapped_nft_id,
            addr2.clone(),
    ...
    router
        .execute_contract(
            ...
            &ExecuteMsg::SetWrappedNftAddress {
                wrapped_nft_address: AndrAddr::from_string(wrapped_nft_addr.to_string()),
            },
            &[],
        )
        .unwrap();

    let origin_nft_mint_msg: cw721_base::msg::ExecuteMsg<Empty, Empty> =
    ...
    let owner_res: OwnerOfResponse = router
        .wrap()
        .query_wasm_smart(&origin_nft_addr, &query_owner_msg)
        .unwrap();
    ...
    let hook_msg = Cw721HookMsg::MintWrappedNft {
        ...
        recipient: Some(AndrAddr::from_string(wrapped_nft_owner.to_string())),
        unwrappable: true,
    };
    let send_origin_nft_msg: cw721_base::msg::ExecuteMsg<Empty, Empty> =
    ...
    let owner_res: OwnerOfResponse = router
        .wrap()
        .query_wasm_smart(&wrapped_nft_addr, &query_owner_msg)
        .unwrap();
    ...
    let send_wrapped_nft_msg: cw721_base::msg::ExecuteMsg<Empty, Empty> =
        cw721_base::msg::ExecuteMsg::SendNft {
            contract: contract_addr.to_string(),
            token_id: "wrapped_token1".to_string(),
            msg: encode_binary(&hook_msg).unwrap(),
    ...
    let res: Result<OwnerOfResponse, StdError> = router
        .wrap()
        .query_wasm_smart(&wrapped_nft_addr, &query_owner_msg);
    res.unwrap_err();
}
